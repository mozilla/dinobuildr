...
if [ "$(echo "$dockutil" | shasum -a 256 | awk '{print $1}')" == $hash ]; then #  if the hashes match then proceed
    ...
    python3 -c "$dockutil" --remove all --no-restart
    python3 -c "$dockutil" --add "/System/Applications/Launchpad.app" --position beginning --no-restart
    python3 -c "$dockutil" --add "/Applications/Firefox.app" --after Launchpad --no-restart
    python3 -c "$dockutil" --add "/Applications/Slack.app" --after Firefox --no-restart
    python3 -c "$dockutil" --add "/Applications/zoom.us.app" --after Slack --no-restart
    python3 -c "$dockutil" --add "~/Downloads" --view fan --display stack --section others --no-restart
    python3 -c "$dockutil" --add "/System/Applications/System Preferences.app" --position end
else 
